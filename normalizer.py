...
import logger
from common.utils import reshape_for_broadcasting


class Normalizer:
    def __init__(self, size, eps=1e-2, default_clip_range=np.inf, sess=None, PLN=False):
    ...
    def update(self, v, isNull=False):
    ...
    def _mpi_average(self, x):
        # buf = np.zeros_like(x)
        buf = MPI.COMM_WORLD.allreduce(x, op=MPI.SUM)
    ...
    def recompute_stats(self, inc=0):
        with self.lock:
            # Copy over results.
