...
COLORS = [ 'red', 'deepskyblue', 'teal','orange', 'darkturquoise', 'springgreen', 'tomato', 'slateblue' , 'salmon', 'gold', 'yellow',  'cyan',
        'blue', 'green', 'black', 'purple', 'pink',
        'brown', 'lightblue', 'lime', 'magenta', 'lavender', 
        'darkgreen', 'tan',  'darkred', 'darkblue']


def default_xy_fn_monitor(r):
    x = np.cumsum(r.monitor.l)
    y = smooth(r.monitor.r, radius=5)
    return x,y

def default_xy_fn(r, fields):
    import re
    x = np.array(r.progress[fields[0]])
    y = smooth(r.progress[fields[1]] , radius=5)
    return x,y

def default_split_fn(r):
    # match name between algorithm and environment_name at the head of the string
    # match = re.search(r'[^/-]+(?=(-\d+)?\Z)', r.dirname)
    dirnm = os.path.split(r.dirname)[-1]
    segs = dirnm.split('-')[0]
    if segs:
        grp = segs.split('_')[0]
        if grp == "hplanner":
...
def plot_results(
    ...
    fields=['train/episode', 'test/success_rate'],
    average_group=False,
    shaded_std=True,
    shaded_err=True,
    figsize=None,
    ...
    groups = sorted(list(set(group_fn(result) for result in allresults)), reverse=True)
    print('Groups:', groups)
